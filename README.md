...
## Wan 2.2 TI2V Action Control (Custom Implementation)

This repo includes a custom Action Control module for **Wan2.2-TI2V-5B**. It adds per-frame action conditioning for I2V training/inference and supports patch-level or layer-level action injection.
...
- **Pipeline wiring**: `diffsynth/pipelines/wan_video.py` (inputs + CFG merge + model_fn injection)
- **Training**: `examples/wanvideo/model_training/train_action.py`
- **Launcher**: `examples/wanvideo/model_training/train_action.sh`
...
video,prompt,action
```
- `video`: relative path to the video file under `dataset_base_path`
- `prompt`: text caption
- `action`: JSON string of shape `(20, 2, 8)` or `(20, 8)`  
  - 20 steps = latent frames 1..20 for `num_frames=81`
  - 8 actions (magnitude only): `[F, B, L, R, YawL, YawR, PitchUp, PitchDown]`
  - If `(20, 2, 8)` is used, only the magnitude channel (index 1) is loaded; binary is ignored.

Magnitude scaling (train + inference must be consistent):
```
...
```
video,prompt,action
clips/000001.mp4,"A drone flying forward", "[[[1,0,0,0,0,0,0,0],[0.02,0,0,0,0,0,0,0]], ...]"
```

Notes:
- `num_frames=81` -> latent frames = 21 -> action length = 20.
- If your CSV has `input_image`, it is currently ignored; training uses the first video frame as `input_image`.

Local model directory format (example):
```
/path/to/Wan2.2-TI2V-5B-local/
  model_index.json
  dit/diffusion_pytorch_model-00001-of-00002.safetensors
  dit/diffusion_pytorch_model-00002-of-00002.safetensors
...
```
Notes:
- Component paths are relative to `--model_dir`.
- Wildcards are supported (e.g., `dit/diffusion_pytorch_model*.safetensors`).

### Training
Main script:
```
python examples/wanvideo/model_training/train_action.py \
  --dataset_base_path /path/to/data \
  --dataset_metadata_path /path/to/clips_metadata.csv \
  --model_dir "/path/to/Wan2.2-TI2V-5B-local" \
  --trainable_models "action_controller" \
  --action_controller_config "wan_ti2v_5b" \
  --action_injection_type "patch_add" \
  --learning_rate 1e-4 \
  --num_epochs 20 \
  --height 480 \
  --width 832 \
  ...
  --output_path ./output_action_controller
```

Notes:
- `--model_dir` must be a local model directory. DiffSynth will load from a components-style index (`diffsynth_model_index.json` or `model_index.json` with a `components` object), or infer common Wan component files from the directory layout.
- Action training does not support LoRA.

Outputs:
```
output_action_controller/
├── action_train_config.json
├── action_controller.safetensors
└── epoch-*.safetensors (optional)
```

Resume training:
- Enable resume checkpoints (lightweight): `--save_full_state --save_steps 1000 --max_checkpoints 5`
- Resume from a checkpoint: `--resume_from ./output_action_controller/checkpoints/step-00001000`

### Inference (with Action)
Load the config + action controller, then pass `action_magnitude`:
```
from diffsynth.pipelines.wan_video import WanVideoPipeline, ModelConfig
from diffsynth.core import load_model_dir
from diffsynth.models.wan_video_action_controller import WanActionController
from safetensors.torch import load_file as load_safetensors
import json, torch

_, components = load_model_dir("/path/to/Wan2.2-TI2V-5B-local")
pipe = WanVideoPipeline.from_pretrained(
    torch_dtype=torch.bfloat16,
    device="cuda",
    model_configs=[
        ModelConfig(path=components["wan_video_dit"]),
        ModelConfig(path=components["wan_video_text_encoder"]),
        ModelConfig(path=components["wan_video_vae"]),
    ],
)

cfg = json.load(open("./output_action_controller/action_train_config.json"))
pipe.action_controller = WanActionController(**cfg["action_controller_config"])
pipe.action_controller.load_state_dict(load_safetensors("./output_action_controller/action_controller.safetensors"))
pipe.action_controller = pipe.action_controller.to(pipe.device, pipe.torch_dtype)

action_magnitude = torch.zeros(1, 20, 8)
action_magnitude[:, :, 0] = 0.02
action_magnitude = action_magnitude * torch.tensor([100, 100, 100, 100, 1, 1, 1, 1])

video = pipe(
    prompt="A drone flying forward",
...
### Notes
- **CFG**: action is passed via `inputs_shared`, so it applies to both positive and negative branches; `cfg_merge=True` is handled by concatenating action tensors.
- **Frame alignment**: action length must match `latent_frames - 1`.
