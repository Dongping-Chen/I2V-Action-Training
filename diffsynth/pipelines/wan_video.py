...
class WanVideoPipeline(BasePipeline):
    ...
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16):
        ...
        self.units = [
            ...
            WanVideoUnit_ActionTextEmbedder(),
            WanVideoUnit_S2V(),
    ...
    def __call__(
        ...
        if inputs_shared["action_magnitude"] is not None and not torch.is_tensor(inputs_shared["action_magnitude"]):
            inputs_shared["action_magnitude"] = torch.tensor(inputs_shared["action_magnitude"], dtype=torch.float32)
        if inputs_shared["action_magnitude"] is not None and inputs_shared["action_magnitude"].dim() == 2:
            inputs_shared["action_magnitude"] = inputs_shared["action_magnitude"].unsqueeze(0)
...
class WanVideoUnit_ActionTextEmbedder(PipelineUnit):
    def __init__(self):
        ...
        self.combo_phrases = []
        for t_idx, t in enumerate(self.translation_phrases):
            for r_idx, r in enumerate(self.rotation_phrases):
                if t_idx == 0 and r_idx == 0:
                    phrase = "Camera remains still."
                elif t_idx == 0:
                    phrase = r
                elif r_idx == 0:
                    phrase = t
                    ...
                    phrase = f"{t} {r}"
                self.combo_phrases.append(phrase)

    def _mean_pool(self, embeds: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        mask = mask.to(dtype=embeds.dtype)
        lengths = mask.sum(dim=1, keepdim=True).clamp(min=1)
        pooled = (embeds * mask.unsqueeze(-1)).sum(dim=1) / lengths
        return pooled

    def _ensure_action_shape(self, action_magnitude: torch.Tensor) -> torch.Tensor:
        if action_magnitude.dim() == 4 and action_magnitude.shape[2] == 2:
            return action_magnitude[:, :, 1, :]
        if action_magnitude.dim() == 3 and action_magnitude.shape[1] == 2 and action_magnitude.shape[2] == 8:
            return action_magnitude.unsqueeze(0)[:, :, 1, :]
        if action_magnitude.dim() == 2:
            return action_magnitude.unsqueeze(0)
        return action_magnitude

    def _action_indices(self, action_magnitude: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        threshold = 1e-6
    ...
    def _get_action_text_table(self, pipe: WanVideoPipeline, device, dtype) -> torch.Tensor:
        cached = getattr(pipe, "_action_text_table", None)
        ...
        pooled = self._mean_pool(embeds, mask).to(dtype=dtype)
        table = pooled.view(9, 9, -1)
    ...
    **kwargs,
    ...
    if action_controller is not None:
        ...
        if action_magnitude is not None:
            ...
            if action_magnitude.dim() == 4 and action_magnitude.shape[2] == 2:
                action_magnitude = action_magnitude[:, :, 1, :]
            elif action_magnitude.dim() == 3 and action_magnitude.shape[1] == 2 and action_magnitude.shape[2] == 8:
                action_magnitude = action_magnitude.unsqueeze(0)[:, :, 1, :]
            ...
            if use_text:
                ...
                action_emb = action_controller.build_action_emb_from_text(action_text_embeds, action_magnitude)
            else:
    ...
    if tea_cache_update:
        ...
        def create_custom_forward(module, block_idx):
                ...
                if use_gradient_checkpointing_offload:
                    with torch.autograd.graph.save_on_cpu():
                        x = torch.utils.checkpoint.checkpoint(
                            create_custom_forward(block, block_id),
